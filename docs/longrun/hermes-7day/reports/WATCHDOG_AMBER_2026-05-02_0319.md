# WATCHDOG AMBER — 2026-05-02 03:19 +08

## Summary

watchdog: AMBER
need_human: false
reason: hard RED gates are clear; disk and Windows-side llama-swap are GREEN, checkpoint cadence is fresh enough, and no current live-root recursive scan or Stage7/downstream writer is present. Persistent WSL OpenClaw gateway/node processes remain active under the quarantine/ban, so US-003/capture/downstream expansion remains blocked.
...
- run_state_last_updated: `2026-05-01T12:05:00+08:00`; logical age ~= 915 min — INFO only because current watchdog/report checkpoint cadence is fresh and daily-executor contract exists
...
- WSL localhost check: connection refused — INFO only due WSL localhost isolation; Windows PowerShell is canonical
- checkpoint_age: 32.8 minutes; latest prior report `WATCHDOG_AMBER_2026-05-02_0246.md` — GREEN for ~30m cadence with minor runtime drift
...
pc         46866  5.6  4.2 19011204 690384 ?     Sl   May01  20:10 /usr/local/bin/node /home/pc/.npm-global/lib/node_modules/openclaw/dist/index.js gateway --port 18789
```

No active recursive `/mnt/d/DDownload` or `/mnt/d/HTML` scan was present in the current sample. No Stage7/canary/downstream broad writer was present in the current sample. Windows-native D-touch broad-operation scan returned empty.
...
```json
{
  ...
  "wsl_localhost_isolation": "INFO: WSL 127.0.0.1 refused; Windows PS HTTP 200 is canonical"
}
```
