...
# WATCHDOG AMBER — 2026-04-30 14:45 +08
...
- WSL curl llama-swap: `curl -s --max-time 5 http://127.0.0.1:11434/v1/models` exited 7 / connection failed, consistent with known WSL2 localhost isolation.
- Windows-side llama-swap: UP (`HTTP 200`, model count 1, models=`qwen3.6:27b`).
- latest reports/checkpoints: latest prior report `WATCHDOG_AMBER_2026-04-30_1413.md` age ~31.8 minutes at sample time; run-state mtime age ~156.8 minutes.
- output dir: `/mnt/d/HTML/hermes-longrun-2026-04-28/` size `0`.
- Windows D-touch process scan: only `D:\agent-memory` local-mem0 processes matched; no Windows-native large `D:\DDownload` / `D:\HTML` file-operation process found.
