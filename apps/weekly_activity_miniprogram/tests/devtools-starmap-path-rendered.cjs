/**
 * Rendered proof: star-map A<->B path-finding. Launches the live star-map, arms
 * path mode from a selected node, then resolves a real relation chain through the
 * backend BFS and asserts the rendered pathResult chain (Knopha -> Cocoonics, 1 hop).
...
async function main() {
  ...
  try {
    ...
    const url = `/pages/atlas-starmap/atlas-starmap?lens=entity&focusId=${encodeURIComponent(fromId)}&renderProofTs=${encodeURIComponent(stamp)}`;
    await step("launch starmap", () => withTimeout(callApp(ws, function launch(u) {
      return new Promise((resolve) => wx.reLaunch({ url: u, success: () => resolve({ ok: true }), fail: (e) => resolve({ ok: false, e }) }));
    }, [url], 20000), 25000, "reLaunch"));

    await sleep(6000);

    // Arm path mode from the focused node, then dispatch a real path query.
    const armed = await step("arm path mode + dispatch", () => withTimeout(callApp(ws, function arm(args) {
      const p = getCurrentPages()[getCurrentPages().length - 1];
      if (!p || p.route !== "pages/atlas-starmap/atlas-starmap") return { ok: false, route: p && p.route };
      p.startPathMode();
      const pathMode = Boolean(p.data.pathMode);
      p._fetchPath(args.fromId, args.fromName, args.toId, args.toName);
      return { ok: true, pathMode };
    }, [{ fromId, fromName, toId, toName }], 15000), 20000, "arm"));
    assert.equal(armed.ok, true, "starmap must be the active page");
    assert.equal(armed.pathMode, true, "startPathMode must arm path mode");

    const state = await step("wait path result", () => withTimeout(pollUntil(
      "path resolve", 30000, 800,
      () => callApp(ws, function readPath() {
        const p = getCurrentPages()[getCurrentPages().length - 1];
        const pr = p && p.data ? p.data.pathResult : null;
        if (!pr) return { status: "" };
        return {
          status: String(pr.status || ""),
          hops: Number(pr.hops || 0),
          summary: String(pr.summary || ""),
          chainNames: Array.isArray(pr.chain) ? pr.chain.map((c) => c && c.name) : [],
          chainRels: Array.isArray(pr.chain) ? pr.chain.map((c) => c && c.rel).filter(Boolean) : [],
        };
      }, [], 8000),
      (s) => s && s.status && s.status !== "loading",
    ), 35000, "path resolve"));

    assert.equal(state.status, "ready", `path must resolve ready, got ${state.status} (${state.summary})`);
    assert.ok(state.hops >= 1, "path must have at least one hop");
    assert.equal(state.chainNames[0], fromName, "chain starts at the from node");
    assert.equal(state.chainNames[state.chainNames.length - 1], toName, "chain ends at the to node");
    assert.ok(state.chainRels.length >= 1, "each hop is labelled with a relation");

    const report = {
      schemaVersion: "devtools_starmap_path_rendered.v1",
      generatedAt: new Date().toISOString(),
      artifactDir, wsEndpoint, baseUrl, query: { fromId, toId },
      state, steps,
...
main().catch((error) => {
  writeJson("failure.json", {
    schemaVersion: "devtools_starmap_path_rendered.failure.v1",
