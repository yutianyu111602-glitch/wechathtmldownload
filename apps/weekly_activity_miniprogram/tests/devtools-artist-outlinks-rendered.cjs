/**
 * Rendered proof: DJ detail page shows accepted external links (outlinks).
 * Asserts: djOutlinks.length >= 1, no 'other' role, fold <= 3, candidateOnly on each.
 * Safety: no upload/deploy/review/release/production DB write.
 */
...
const targetSubjectId = String(process.env.ATLAS_RENDER_ARTIST_SUBJECT_ID || "dj:shanghaiqiutian").trim();
const steps = [];
...
async function main() {
  ...
  try {
    ...
    const state = await step("read outlinks state", () => withTimeout(callApp(ws, function readOutlinks() {
      const p = getCurrentPages()[getCurrentPages().length - 1];
      const d = p && p.data ? p.data : {};
      const links = Array.isArray(d.djOutlinks) ? d.djOutlinks : [];
      const visible = Array.isArray(d.visibleDjOutlinks) ? d.visibleDjOutlinks : [];
      return {
        route: p && p.route,
        loading: Boolean(d.loading),
        djOutlinksCount: links.length,
        visibleCount: visible.length,
        hiddenOutlinkCount: Number(d.hiddenOutlinkCount || 0),
        outlinksExpanded: Boolean(d.outlinksExpanded),
        roles: links.map((l) => l && l.role),
        platforms: links.map((l) => l && l.platform),
        candidateFlags: links.map((l) => l && l.candidateOnly),
        firstUrl: links.length > 0 ? links[0].url : "",
      };
    }, [], 15000), 20000, "read outlinks"));
    ...
    assert.ok(state.djOutlinksCount >= 1, `expected djOutlinks >= 1 for ${targetName}, got ${state.djOutlinksCount}`);
    assert.ok(state.visibleCount <= 3, `visible outlinks must be <= fold limit 3, got ${state.visibleCount}`);
    assert.ok(!state.roles.includes("other"), "no other-role links in djOutlinks (gate must filter)");
    assert.ok(state.candidateFlags.every((f) => f === true), "all outlinks must have candidateOnly:true");
    assert.ok(state.firstUrl && state.firstUrl.startsWith("http"), `first outlink url must be valid http, got: ${state.firstUrl}`);

    const report = {
      schemaVersion: "devtools_artist_outlinks_rendered.v1",
...
main().catch((error) => {
  const failure = {
    schemaVersion: "devtools_artist_outlinks_rendered.failure.v1",
    generatedAt: new Date().toISOString(),
    artifactDir,
    wsEndpoint,
    runtimeContext,
