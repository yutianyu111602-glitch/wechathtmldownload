...
// Strong genre evidence may override a mixed bill. Generic venue/event words and
// bare "dub"/"bass" are deliberately kept in the weak tier: they must not turn
// an explicitly jazz, band, funk, or swing event into an electronic event.
...
export const ELECTRONIC_RELEVANCE_HINTS = Object.freeze([
  STRONG_ELECTRONIC_PATTERN_SOURCE,
  WEAK_ELECTRONIC_PATTERN_SOURCE,
]);
export const NON_ELECTRONIC_RELEVANCE_HINTS = Object.freeze(
...
export function activityRelevanceText(item) {
...
export function nonElectronicExclusionReason(item) {
...
export function classifyElectronicMusicRelevance(item) {
  const text = activityRelevanceText(item);
...
export function isElectronicMusicRelevantItem(item) {
  return classifyElectronicMusicRelevance(item) !== "non_electronic";
}

export function electronicRelevanceContractFingerprint() {
