#!/usr/bin/env python3
"""Run the S150 guarded DB3 relation identity canary selected by S149.

S150 consumes the S149 live post-canary queue, binds a separate approval
artifact for exactly one row, and may execute that one DB3 merge under the S148
single-writer/backup/readback implementation. It does not project DB2, rebuild
release packages, deploy, upload, or submit review.
"""
...
CURRENT_STORY_ID = "S150"
SCHEMA_VERSION = "atlas_relation_identity_db3_canary_s150.v1"
APPROVAL_SCHEMA_VERSION = "atlas_relation_identity_operator_approval_s150.v1"

DEFAULT_S149_REPORT = (
    REPORTS_ROOT
    / "atlas_relation_identity_post_canary_refresh_s149_20260602"
    / "atlas_relation_identity_post_canary_refresh_s149.json"
)
DEFAULT_DB3 = REPO_ROOT / "services" / "weekly_activity_cloudrun" / "data" / "atlas_miniapp.sqlite"
DEFAULT_OUT_DIR = REPORTS_ROOT / "atlas_relation_identity_db3_canary_s150_20260602"
DEFAULT_APPROVAL_ARTIFACT = DEFAULT_OUT_DIR / "operator_approval_s150.json"
DEFAULT_SCORECARD = REPO_ROOT / "reports" / "WEEKLY_ATLAS_RELATION_IDENTITY_DB3_CANARY_S150_20260602.md"
...
def selected_from_s149(s149_report: dict[str, Any]) -> dict[str, Any]:
    selected = s149_report.get("next_canary") if isinstance(s149_report.get("next_canary"), dict) else {}
    if not selected:
        return {}
...
def ensure_approval(
    *,
    s149_report: dict[str, Any],
    s149_report_path: Path,
    approval_path: Path,
    create: bool,
    operator: str,
) -> dict[str, Any]:
    ...
    selected = selected_from_s149(s149_report)
    payload = {
        "schema_version": APPROVAL_SCHEMA_VERSION,
        "current_story_id": CURRENT_STORY_ID,
        "approval_status": "approved",
        "approval_scope": "single_s150_canary_only",
        "source_report_sha256": sha256_file(s149_report_path),
        ...
        "operator": operator,
        "approved_at": now_iso(),
        "approval_statement": "User granted production write/autonomy; approve only this S150 single-row DB3 canary selected by S149.",
        "approval_basis": {
            "s149_decision": s149_report.get("decision"),
            "s148_closure_ok": bool((s149_report.get("s148_closure") or {}).get("ok")),
...
def validate_approval(
    *,
    ...
    gaps: list[str] = []
    if not selected:
        return False, ["s149_next_canary_missing"]
    if approval is None:
    ...
    if s149_report.get("decision") != "atlas_relation_identity_post_canary_refresh_s149_ready_next_canary_report_only":
        gaps.append("s149_decision_not_ready")
    if (s149_report.get("s148_closure") or {}).get("ok") is not True:
        gaps.append("s148_closure_not_ok")
...
def make_tasks(report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if (report.get("write_state") or {}).get("committed"):
        return [
            {
                **base,
                "task_id": "s150:postwrite_relation_refresh",
                "status": "next",
                "next_action": "Run the post-S150 live relation refresh to prove the selected group is closed and select the next bounded canary.",
            }
        ]
    return [
        {
            **base,
            "task_id": "s150:canary_blocked_before_commit",
            "status": "blocked",
            "blockers": (report.get("operator_approval") or {}).get("validation_gaps")
            or (report.get("execution") or {}).get("blocked_reasons")
            or [],
            "next_action": "Repair approval or execution blockers, then rerun S150 on the same selected canary only.",
        }
    ]
...
def build_report(
    *,
    ...
    s149_report = read_json(s149_report_path)
    selected = selected_from_s149(s149_report)
    approval_creation = ensure_approval(
        s149_report=s149_report,
        s149_report_path=s149_report_path,
    ...
    if not approval_valid:
        decision = "atlas_relation_identity_db3_canary_s150_blocked_invalid_or_missing_approval"
    elif execution.get("committed"):
        decision = "atlas_relation_identity_db3_canary_s150_committed_with_readback"
    elif execute:
        decision = "atlas_relation_identity_db3_canary_s150_blocked_before_commit"
    else:
        decision = "atlas_relation_identity_db3_canary_s150_approval_valid_no_write_dry_run"
    report: dict[str, Any] = {
        ...
        "decision": decision,
        "source_inputs": {
            "s149_report": rel_path(s149_report_path),
...
def render_markdown(report: dict[str, Any], paths: dict[str, str]) -> str:
    ...
    lines = [
        "# Atlas Relation Identity DB3 Canary S150",
        "",
        ...
        "## Boundary",
        "",
        "- S150 touches only the selected S149 canary row after approval validation.",
        "- DB2 projection, release rebuild, deploy, upload, and review remain blocked.",
        "- Other S149 queue rows and S145 gap rows remain excluded.",
        "",
    ]
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--s149-report", type=Path, default=DEFAULT_S149_REPORT)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        s149_report_path=args.s149_report,
    ...
    paths = {
        "json": rel_path(args.out_dir / "atlas_relation_identity_db3_canary_s150.json"),
        "readback": rel_path(args.out_dir / "relation_identity_db3_canary_readback_s150.json"),
        "tasks": rel_path(args.out_dir / "relation_identity_db3_canary_tasks_s150.jsonl"),
        "markdown": rel_path(args.out_dir / "atlas_relation_identity_db3_canary_s150.md"),
        "scorecard": rel_path(args.scorecard),
    }
    atomic_write_json(args.out_dir / "atlas_relation_identity_db3_canary_s150.json", report)
    atomic_write_json(args.out_dir / "relation_identity_db3_canary_readback_s150.json", report.get("execution") or {})
    atomic_write_jsonl(args.out_dir / "relation_identity_db3_canary_tasks_s150.jsonl", report["next_action_tasks"])
    atomic_write_text(args.out_dir / "atlas_relation_identity_db3_canary_s150.md", render_markdown(report, paths))
