#!/usr/bin/env python3
"""Build a report-only source-date acceptance/write-gate contract packet."""
...
DEFAULT_READBACK_DIR = STAGE7_ROOT / "reports" / "atlas_social_manual_participant_source_date_readback_gate_q6_20260526"
DEFAULT_READY_ROWS = DEFAULT_READBACK_DIR / "source_date_readback_ready_report_only.jsonl"
DEFAULT_READBACK_SUMMARY = DEFAULT_READBACK_DIR / "source_date_readback_gate_summary.json"
...
DEFAULT_OUT_DIR = STAGE7_ROOT / "reports" / "atlas_social_manual_participant_source_date_acceptance_write_gate_q6_20260526"
DEFAULT_REPORT = REPO_ROOT / "reports" / "ATLAS_T6_MANUAL_PARTICIPANT_SOURCE_DATE_ACCEPTANCE_WRITE_GATE_20260526.md"
SCHEMA_VERSION = "stage7_atlas_social_manual_participant_source_date_acceptance_write_gate.v1"
...
def reject_d_path(path: Path, label: str) -> None:
    ...
    if raw.startswith("d:/") or raw.startswith("/mnt/d/"):
        raise ValueError(f"{label} must not point to D: for source-date acceptance write gate: {path}")
...
def row_blockers(row: dict[str, Any]) -> list[str]:
    ...
    selected_date = compact(row.get("selected_date_report_only"), 80)
    counts_by_event = participant_counts(row)

    if compact(row.get("readback_status")) != "source_date_context_db_readback_gate_ready_report_only":
    ...
    if not selected_date:
        blockers.append("selected_date_missing")
    if not event_ids:
    ...
    if selected_date not in set(list_strings(performance.get("date_values"), 80)):
        blockers.append("selected_date_not_in_readback_date_values")
    for key in [
        "performance_event_missing_event_ids",
        "selected_event_date_mismatch_event_ids",
...
def build_target(row: dict[str, Any], generated_at: str, target_db_ready: bool) -> dict[str, Any]:
    ...
    selector_id = stable_id(
        "sourcedateacceptgate",
        [
            row.get("source_date_readback_selector_id"),
            row.get("article_uid"),
            selected_date,
            "|".join(event_ids),
            "|".join(source_refs),
    ...
    ready_for_manual_acceptance = not row_level_blockers
    source_raw_target_db_ready_row = ready_for_manual_acceptance and target_db_ready
    blockers = sorted(set(row_level_blockers + target_db_blockers))
    participant_by_event = participant_counts(row)
    non_representative_event_ids = event_ids[1:]

    return {
        "schema_version": f"{SCHEMA_VERSION}.target_row",
        "generated_at": generated_at,
        "source_date_acceptance_gate_id": selector_id,
        "source_date_readback_selector_id": compact(row.get("source_date_readback_selector_id"), 180),
        "source_date_context_review_id": compact(row.get("source_date_context_review_id"), 180),
        ...
        "resolution_reason": sanitize_report_text(row.get("resolution_reason"), 260),
        ...
        "blockers": blockers,
        "manual_date_context_acceptance_precheck_ready": ready_for_manual_acceptance,
        "source_raw_target_db_provenance_ready_report_only": source_raw_target_db_ready_row,
        ...
        "prewrite_snapshot_contract": {
            ...
            "must_capture_tables": [
                "source/raw event fact table for selected event ids",
        ...
        "dry_run_transaction_work_order": {
            ...
            "operation": "manual_date_context_acceptance_report_only",
            "would_bind_article_uid": compact(row.get("article_uid"), 260),
            "would_bind_selected_date": selected_date,
        ...
        batches.append(
            {
                ...
                "manual_date_context_acceptance_precheck_ready_rows": sum(
                    1 for row in group if row.get("manual_date_context_acceptance_precheck_ready") is True
                ),
                ...
                "selected_dates": sorted({compact(row.get("selected_date_report_only"), 80) for row in group if row.get("selected_date_report_only")}),
                "write_execution_allowed_now": False,
            }
        )
    return batches
...
def build_packet(
    ...
    manual_ready_rows = [row for row in targets if row["manual_date_context_acceptance_precheck_ready"]]
    ...
    dry_run_rows = [row["dry_run_transaction_work_order"] | {"source_date_acceptance_gate_id": row["source_date_acceptance_gate_id"]} for row in targets]
    rollback_rows = [row["rollback_contract"] | {"source_date_acceptance_gate_id": row["source_date_acceptance_gate_id"]} for row in targets]
    postwrite_rows = [row["postwrite_readback_contract"] | {"source_date_acceptance_gate_id": row["source_date_acceptance_gate_id"]} for row in targets]

    counts = {
        "input_ready_rows": len(rows),
        "write_gate_target_rows": len(targets),
        "manual_date_context_acceptance_precheck_ready_rows": len(manual_ready_rows),
        ...
        "unique_selected_dates": len({row["selected_date_report_only"] for row in targets if row["selected_date_report_only"]}),
    ...
    decision = (
        "atlas_social_manual_participant_source_date_acceptance_write_gate_ready_report_only"
        if not failed_checks
        else "atlas_social_manual_participant_source_date_acceptance_write_gate_blocked_report_only"
    )

    out_dir.mkdir(parents=True, exist_ok=True)
    outputs = {
        "targets": out_dir / "source_date_acceptance_write_gate_targets.jsonl",
        "manual_acceptance_ready": out_dir / "manual_date_context_acceptance_ready_report_only.jsonl",
        "source_raw_target_db_blocked_rows": out_dir / "source_raw_target_db_blocked_rows.jsonl",
        "row_blocked_rows": out_dir / "source_date_acceptance_row_blocked_rows.jsonl",
        "dry_run_work_orders": out_dir / "source_date_acceptance_dry_run_work_orders.jsonl",
        "rollback_contracts": out_dir / "source_date_acceptance_rollback_contracts.jsonl",
        "postwrite_contracts": out_dir / "source_date_acceptance_postwrite_readback_contracts.jsonl",
        "source_account_batches": out_dir / "source_account_source_date_acceptance_batches.jsonl",
        "summary_json": out_dir / "source_date_acceptance_write_gate_summary.json",
        "summary_md": out_dir / "source_date_acceptance_write_gate_summary.md",
    }
    ...
    if any(leak_counts.values()):
        ...
        decision = "atlas_social_manual_participant_source_date_acceptance_write_gate_blocked_report_only"
    ...
    summary = {
        ...
        "inputs": {
            "source_date_readback_ready_rows": display_path(ready_rows_path),
            "source_date_readback_summary": display_path(readback_summary_path),
        ...
        "wait_reason": (
            "Manual date-context rows are acceptance-precheck ready, but source/raw DB provenance remains missing; "
        ...
        "next_cursor_split": {
            "manual_date_context_acceptance_ready": display_path(outputs["manual_acceptance_ready"]),
            "source_raw_target_db_blocked": display_path(outputs["source_raw_target_db_blocked_rows"]),
            "span_split_review_closed_upstream": "tools/stage7_rewrite/reports/atlas_social_manual_participant_source_date_context_recovery_q6_20260526/overnight_or_span_review_required_rows.jsonl",
        },
    }
...
def render_summary_md(summary: dict[str, Any]) -> str:
    ...
    return (
        "# Source-Date Acceptance Write Gate Summary\n\n"
        f"- Decision: `{summary['decision']}`\n"
        f"- Failed checks: `{summary['failed_checks']}`\n"
        f"- Targets: `{counts['write_gate_target_rows']}`\n"
        f"- Manual acceptance precheck ready: `{counts['manual_date_context_acceptance_precheck_ready_rows']}`\n"
...
def render_report(summary: dict[str, Any]) -> str:
    ...
    return f"""# Atlas T6 Manual Participant Source-Date Acceptance Write Gate - 2026-05-26

## Decision

- Decision: `{summary['decision']}`
- Failed checks: `{summary['failed_checks']}`
- Boundary: report-only manual date-context acceptance/source-raw DB provenance write-gate contract. It does not open source/raw Atlas DB, write source/raw DB, rebuild or write serving SQLite, accept graph facts, write Neo4j/Qdrant/SQLite production state, update public pointers, deploy CloudRun/VPS, upload/review mini-program, execute OCR/network/model calls, or write memory.

## Counts

- Input readback-ready rows: `{counts['input_ready_rows']}`
- Write-gate target rows: `{counts['write_gate_target_rows']}`
- Manual date-context acceptance precheck ready rows: `{counts['manual_date_context_acceptance_precheck_ready_rows']}`
...
- Unique selected event ids / dates: `{counts['unique_selected_event_ids']}/{counts['unique_selected_dates']}`
...
Manual date-context rows are ready only at report-only contract level. Keep write execution closed until a later explicit source/raw target DB real snapshot/write gate supplies target DB provenance, prewrite row hashes, inverse rollback, and postwrite readback evidence.
"""
