#!/usr/bin/env python3
"""Run the confirmed Atlas T5 source/raw city writer with rollback evidence.

Default mode is a dry-run. Mutation mode requires an explicit confirm token and
only updates `events.city` for rows already materialized by the city write
preflight packet.
"""
...
from tools.stage7_rewrite.scripts import build_atlas_t5_city_write_preflight_packet as preflight

PREFLIGHT_DIR = STAGE7_ROOT / "reports" / "atlas_t5_city_write_preflight_20260527"
DEFAULT_READY_ROWS = PREFLIGHT_DIR / "city_write_preflight_ready_raw_event_rows.jsonl"
DEFAULT_PREFLIGHT_SUMMARY = PREFLIGHT_DIR / "city_write_preflight_summary.json"
DEFAULT_PREFLIGHT_CONTRACT = PREFLIGHT_DIR / "city_write_preflight_contract.json"
DEFAULT_TARGET_DB = (
    REPO_ROOT
    / "reports"
...
DEFAULT_OUT_DIR = STAGE7_ROOT / "reports" / "atlas_t5_city_write_execution_gate_20260527"
DEFAULT_REPORT = REPO_ROOT / "reports" / "ATLAS_T5_CITY_WRITE_EXECUTION_GATE_20260527.md"
CONFIRM_TOKEN = "ENABLE_ATLAS_T5_CITY_EVENTS_CITY_WRITE"
SCHEMA_VERSION = "stage7_atlas_t5_city_write_execution_gate.v1"
...
def read_json(path: Path, label: str) -> dict[str, Any]:
    preflight.reject_d_root(path, label)
...
def open_rw(path: Path) -> sqlite3.Connection:
    preflight.reject_d_root(path, "target_db")
    if not path.exists():
...
def raw_event_by_pk(conn: sqlite3.Connection, row_pk: int) -> dict[str, Any] | None:
    ...
    item["prewrite_row_hash"] = preflight.row_hash(item)
    return item
...
def contract_hash_matches(row: dict[str, Any]) -> bool:
    ...
    return preflight.row_hash(payload) == expected


def non_city_mismatches(expected: dict[str, Any], actual: dict[str, Any]) -> list[str]:
    fields = [
        ("raw_event_evid", "evid"),
        ("raw_event_name", "name"),
...
def validate_inputs(
    ...
    if preflight_summary.get("decision") != "atlas_t5_city_write_preflight_partial_ready_report_only":
    ...
    if (preflight_summary.get("counts") or {}).get("raw_event_update_target_rows") != len(ready_rows):
        summary_failures.append("ready_row_count_mismatch_against_preflight_summary")
    if (preflight_summary.get("target_db") or {}).get("display") != preflight.display_path(target_db):
        summary_failures.append("target_db_display_mismatch_against_preflight_summary")
    if (preflight_contract.get("target_db") or {}).get("display") != preflight.display_path(target_db):
        summary_failures.append("target_db_display_mismatch_against_preflight_contract")
    if (preflight_contract.get("target_db") or {}).get("schema_sha256") != preflight.schema_hash(conn):
    ...
    if table_exists(conn, "events"):
        missing_cols = sorted(required_cols - preflight.table_columns(conn, "events"))
        if missing_cols:
    ...
    for index, row in enumerate(ready_rows):
        ...
        proposed_city = preflight.compact(row.get("proposed_city"), 120)
        if row.get("schema_version") != "stage7_atlas_t5_city_write_preflight.v1.ready_raw_event_city_row":
            blockers.append("ready_row_schema_version_unexpected")
        if row.get("target_table") != "events" or row.get("target_column") != "city":
            blockers.append("ready_row_target_not_events_city")
        if row.get("target_db") != preflight.display_path(target_db):
        ...
        if preflight.scan_payload({"proposed_city": proposed_city}) != {
            "public_url_hits": 0,
            "sensitive_key_hits": 0,
            "local_path_hits": 0,
        }:
            blockers.append("proposed_city_leak_shape")
        if row.get("current_city") not in ("", None):
            blockers.append("ready_row_current_city_not_empty")
        if row.get("prewrite_snapshot_required") is not True:
            blockers.append("prewrite_snapshot_not_required")
        for gate in [
        ...
        rollback = row.get("rollback_contract") if isinstance(row.get("rollback_contract"), dict) else {}
        if rollback.get("required") is not True:
            blockers.append("rollback_contract_not_required")
        if rollback.get("restore_value") not in ("", None):
            blockers.append("rollback_restore_value_not_empty")
        if str(rollback.get("prewrite_row_hash") or "") != str(row.get("prewrite_row_hash") or ""):
            blockers.append("rollback_hash_mismatch")
        postwrite = row.get("postwrite_readback_contract") if isinstance(row.get("postwrite_readback_contract"), dict) else {}
        ...
        if actual is None:
            ...
            if preflight.compact(actual.get("city"), 120) != "":
                blockers.append("target_city_not_empty")
            mismatches = non_city_mismatches(row, actual)
            if mismatches:
                blockers.append("non_city_field_drift:" + ",".join(mismatches))

        snapshot = {
            "schema_version": SCHEMA_VERSION + ".prewrite_snapshot",
            "generated_at": generated_at,
            "selector_id": preflight.compact(row.get("selector_id"), 120),
            "raw_event_row_pk": row_pk,
            "expected_prewrite_row_hash": preflight.compact(row.get("prewrite_row_hash"), 128),
            "actual_prewrite_row_hash": preflight.compact(actual_for_output.get("prewrite_row_hash"), 128),
            "current_city": preflight.compact(actual_for_output.get("city"), 120),
            "proposed_city": proposed_city,
            "hash_match": not any(blocker.startswith("prewrite_row_hash_drift") for blocker in blockers)
            and actual is not None,
            "non_city_mismatches": [b for b in blockers if b.startswith("non_city_field_drift:")],
        }
    ...
    *,
    ...
    try:
        ...
        for row in rows:
            ...
            elif preflight.compact(before.get("city"), 120) != "":
                blockers.append("target_city_not_empty_at_write_time")
            ...
            if execute:
                cursor = conn.execute(
                    "UPDATE events SET city = ? WHERE row_pk = ? AND coalesce(city, '') = ''",
            ...
            city_matches = after is not None and preflight.compact(after.get("city"), 120) == proposed_city
            non_city_drift = non_city_mismatches(row, after or {}) if after else ["target_row_missing_after_write"]
            postwrite = {
                "schema_version": SCHEMA_VERSION + ".postwrite_readback",
                "generated_at": generated_at,
                "selector_id": row["selector_id"],
                "raw_event_row_pk": row_pk,
                "expected_city": proposed_city,
                "actual_city": preflight.compact((after or {}).get("city"), 120),
                "city_matches": city_matches if execute else False,
                "non_city_drift_fields": non_city_drift,
                "postwrite_row_hash": preflight.compact((after or {}).get("prewrite_row_hash"), 128),
                "write_executed": execute,
            }
            postwrite_rows.append(postwrite)
            if execute and (not city_matches or non_city_drift):
                blocked.append(
                    {
                        "schema_version": SCHEMA_VERSION + ".write_blocked_row",
                        "generated_at": generated_at,
            ...
            write_rows.append(
                {
                    ...
                    "selector_id": row["selector_id"],
                    "raw_event_row_pk": row_pk,
                    "restore_city": row.get("current_city") or "",
                    "proposed_city": proposed_city,
    ...
    if execute and not committed:
        # Re-read after rollback so the evidence cannot be mistaken for a committed write.
        postwrite_rows = []
        for row in rows:
            ...
            postwrite_rows.append(
                {
                    ...
                    "expected_city": preflight.compact(row.get("proposed_city"), 120),
                    "actual_city": preflight.compact((after or {}).get("city"), 120),
                    "city_matches": False,
                    "non_city_drift_fields": [],
                    "postwrite_row_hash": preflight.compact((after or {}).get("prewrite_row_hash"), 128),
                    "write_executed": False,
    ...
    return [
        {
            ...
            "target_column": "city",
            "raw_event_row_pk": int(row["raw_event_row_pk"]),
            "restore_value": row.get("current_city") or "",
            "written_value": preflight.compact(row.get("proposed_city"), 120),
            "prewrite_row_hash": row["prewrite_row_hash"],
...
def build_summary(
    *,
    ...
    target_display = preflight.display_path(target_db)

    with open_rw(target_db) as conn:
        ...
        write_rows, postwrite_rows, write_blocked_rows, committed = execute_city_write(
            conn,
            ready_rows,
            generated_at,
    ...
    output_scan = preflight.merge_scan(
        preflight.scan_payload(prewrite_rows),
        preflight.scan_payload(write_rows),
        preflight.scan_payload(postwrite_rows),
        preflight.scan_payload(rollback),
        preflight.scan_payload(blocked_rows),
    )
    ...
    counts = {
        ...
        "postwrite_city_match_rows": sum(1 for row in postwrite_rows if row.get("city_matches") is True),
    ...
    decision = (
        "atlas_t5_city_write_execution_gate_source_raw_city_write_verified"
        if execute and committed and not failed_checks
        else "atlas_t5_city_write_execution_gate_dry_run_ready"
        if not execute and ready_rows and not failed_checks
        else "atlas_t5_city_write_execution_gate_blocked"
    )
    outputs = {
        "summary_json": preflight.display_path(out_dir / "city_write_execution_summary.json"),
        "summary_md": preflight.display_path(out_dir / "city_write_execution_summary.md"),
        "prewrite_snapshots": preflight.display_path(out_dir / "city_write_execution_prewrite_snapshots.jsonl"),
        "write_rows": preflight.display_path(out_dir / "city_write_execution_write_rows.jsonl"),
        "postwrite_readback": preflight.display_path(out_dir / "city_write_execution_postwrite_readback.jsonl"),
        "rollback_contracts": preflight.display_path(out_dir / "city_write_execution_rollback_contracts.jsonl"),
        "blocked_rows": preflight.display_path(out_dir / "city_write_execution_blocked_rows.jsonl"),
        "report": preflight.display_path(report_path),
    }
    boundary_truth = {
        "report_only": not committed,
        "source_raw_db_opened_read_write": True,
        "source_raw_db_write_executed": committed,
        "source_raw_db_write_scope": "events.city only" if committed else "none",
    ...
    summary = {
        ...
        "counts": counts,
        "inputs": {
            "ready_rows": preflight.display_path(ready_rows_path),
            "preflight_summary": preflight.display_path(preflight_summary_path),
            "preflight_contract": preflight.display_path(preflight_contract_path),
        ...
        "next_resume_pointer": (
            preflight.display_path(out_dir / "city_write_execution_postwrite_readback.jsonl")
            if committed
            else preflight.display_path(out_dir / "city_write_execution_blocked_rows.jsonl")
            if blocked_rows
            else preflight.display_path(out_dir / "city_write_execution_write_rows.jsonl")
        ),
        "next_if_write_verified": preflight.display_path(REPO_ROOT / "reports" / "atlas_serving_participant_delta_current_20260526_0016" / "atlas_serving.sqlite"),
        "next_if_blocked": preflight.display_path(
            STAGE7_ROOT
            / "reports"
    ...
    write_jsonl(out_dir / "city_write_execution_prewrite_snapshots.jsonl", prewrite_rows)
    write_jsonl(out_dir / "city_write_execution_write_rows.jsonl", write_rows)
    write_jsonl(out_dir / "city_write_execution_postwrite_readback.jsonl", postwrite_rows)
    write_jsonl(out_dir / "city_write_execution_rollback_contracts.jsonl", rollback)
    write_jsonl(out_dir / "city_write_execution_blocked_rows.jsonl", blocked_rows)
    write_json(out_dir / "city_write_execution_summary.json", summary)
    write_text(out_dir / "city_write_execution_summary.md", render_summary(summary))
...
def render_summary(summary: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Atlas T5 City Write Execution Summary",
            "",
            ...
            f"- Postwrite readback / city match rows: `{counts['postwrite_readback_rows']}/{counts['postwrite_city_match_rows']}`",
...
def render_report(summary: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# ATLAS T5 City Write Execution Gate - 2026-05-27",
            "",
            "## Decision",
            "",
            f"`{summary['decision']}`",
            ...
            "",
            (
                "This writer is intentionally narrow. It accepts only rows produced by the prior city "
                "preflight packet, rechecks the source/raw target schema and every prewrite row hash, "
                "then updates only `events.city`. The write does not rebuild serving SQLite and does "
                "not promote graph/vector/public fields."
            ),
            "",
            "## Counts",
            "",
            ...
            f"- Write committed rows: `{counts['write_committed_rows']}`",
            f"- Postwrite readback / city matches: `{counts['postwrite_readback_rows']}` / `{counts['postwrite_city_match_rows']}`",
            ...
            (
                ...
                "updated source/raw DB. If blocked, continue the time-title/source-OCR queues."
            ),
            "",
        ]
    )
