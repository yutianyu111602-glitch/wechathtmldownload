# Stage 7 Rewrite — Canary 5 Runner (Python replacement)
# Runs doctor, audit, build-manifest, run-llm for 5 articles.
...
$LOG_FILE = "$LOG_DIR\canary5.log"

New-Item -ItemType Directory -Force -Path $LOG_DIR | Out-Null
Set-Location $STAGE7_ROOT

Write-Host "=== Stage 7 Canary 5 (Python Runner) ==="
...
Write-Host ""
Write-Host "[3/4] Build manifest (canary, limit 5)..."
python -m stage7.cli build-manifest --mode canary --limit 5 --output "$OUTPUT_ROOT" 2>&1 | Tee-Object -FilePath $LOG_FILE -Append
if ($LASTEXITCODE -ne 0) {
...
Write-Host ""
Write-Host "[4/4] Run LLM (canary, limit 5) — NO --allow-empty-recovery..."
python -m stage7.cli run-llm --mode canary --limit 5 --output "$OUTPUT_ROOT" 2>&1 | Tee-Object -FilePath $LOG_FILE -Append
if ($LASTEXITCODE -ne 0) {
...
Write-Host ""
Write-Host "=== Canary 5 complete ==="
Write-Host "Report: $OUTPUT_ROOT\reports\CANARY_REPORT.md"
Write-Host "Log   : $LOG_FILE"
exit 0
