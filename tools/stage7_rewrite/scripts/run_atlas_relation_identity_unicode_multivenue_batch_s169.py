#!/usr/bin/env python3
"""Run a guarded DB3 identity batch for Unicode multi-venue name variants.

S169 consumes the post-S167 same-normalized backlog and opens a stricter
venue-only gate: two profiles in the same non-empty city, Unicode compact name
equivalence, no shared source/event, at least two shared venue anchors, and no
collective/lineup/role/collaboration risk terms. The write path is DB3-only
with lock, backup, transaction, redirect, and precommit/postcommit readback.
"""
...
CURRENT_STORY_ID = "S169"
REPORT_STEM = "atlas_relation_identity_unicode_multivenue_batch_s169"
SCHEMA_VERSION = f"{REPORT_STEM}.v1"

DEFAULT_DB3 = REPO_ROOT / "services" / "weekly_activity_cloudrun" / "data" / "atlas_miniapp.sqlite"
DEFAULT_OUT_DIR = REPORTS_ROOT / "atlas_relation_identity_unicode_multivenue_batch_s169_20260602"
DEFAULT_SCORECARD = REPO_ROOT / "reports" / "WEEKLY_ATLAS_RELATION_IDENTITY_UNICODE_MULTIVENUE_BATCH_S169_20260602.md"
...
def profile_evidence_ok(candidate: dict[str, Any]) -> bool:
    rows = [candidate.get("canonical_profile") or {}, *(candidate.get("merge_profiles") or [])]
    return all(
        int(row.get("subject_present") or 0) == 1
        and int(row.get("event_rows_with_source_ref") or 0) > 0
...
def live_candidates(conn: sqlite3.Connection) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    ...
    for (city, token), group in sorted(s164.grouped_profile_rows(conn).items()):
        ...
        if int(classified["common_source_ref_count"]) != 0:
            blockers.append("common_source_ref_belongs_to_s167")
        if int(classified["common_event_count"]) != 0:
            blockers.append("common_event_requires_separate_gate")
        if int(classified["common_venue_count"]) < MIN_SHARED_VENUES:
            blockers.append("common_venue_count_below_2")
        if s167.has_enhanced_risk(values):
            blockers.append("enhanced_risk_or_collaboration_marker")
        if blockers:
            rejected.append({**classified, "s169_blockers": sorted(set(blockers)), "unicode_compact_values": sorted(compacts)})
            continue
        candidate = s159.build_candidate(city, token, group)
        candidate["prewrite_row_id"] = f"s169:{s159.group_hash(city, token, ids)}"
        candidate["s159_rejection_reasons_before_s169"] = classified["s159_rejection_reasons"]
        ...
        candidate["safety_basis"] = [
            ...
            "at_least_two_common_venue_anchors",
            "enhanced_collective_lineup_role_and_collaboration_risk_absent",
        ]
        if not profile_evidence_ok(candidate):
            rejected.append({**classified, "s169_blockers": ["profile_source_ref_or_subject_evidence_missing"], "unicode_compact_values": sorted(compacts)})
            continue
        rows.append(candidate)
    state = {
        "same_normalized_multi_id_group_count": len(rows) + len(rejected),
        "unicode_multivenue_candidate_count": len(rows),
...
def validate_s169_prewrite(conn: sqlite3.Connection, selected: dict[str, Any]) -> list[str]:
    blockers: list[str] = []
    ...
    if not selected.get("city_key"):
        blockers.append("city_empty")
    if not set(selected.get("s159_rejection_reasons_before_s169") or []) <= ALLOWED_REASONS:
    ...
    if int(selected.get("shared_event_count") or 0) != 0:
        blockers.append("common_event_requires_separate_gate")
    if int(selected.get("shared_venue_count") or 0) < MIN_SHARED_VENUES:
        blockers.append("common_venue_count_below_2")
...
def execute_selected(
    *,
    ...
    lock_path = out_dir / f"atlas_relation_identity_unicode_multivenue_batch_{story_label}.lock"
    result: dict[str, Any] = {
    ...
    with s148.FileLock(lock_path, max_attempts=max_lock_attempts):
        if shared_backup:
            ...
            backup_path = out_dir / f"backup_before_relation_identity_unicode_multivenue_{story_label}_{stamp}.sqlite"
        ...
        try:
            pre_blockers = validate_s169_prewrite(conn, selected)
            if pre_blockers:
...
def run_batch(
    *,
    ...
    if execute and batch_backup:
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_path = out_dir / f"backup_before_relation_identity_unicode_multivenue_batch_s169_{stamp}.sqlite"
        shutil.copy2(db3_path, backup_path)
    ...
    for index in range(max_count):
        ...
        if not queue:
            ...
            break
        selected = queue[0]
        story_label = f"s169_{index + 1:03d}"
...
def write_markdown(path: Path, report: dict[str, Any]) -> None:
    lines = [
        "# Weekly Atlas Relation Identity Unicode Multivenue Batch S169",
        "",
        ...
        f"- Initial eligible candidates: `{report['initial_state']['unicode_multivenue_candidate_count']}`",
        f"- Final eligible candidates: `{report['final_state']['unicode_multivenue_candidate_count']}`",
        ...
        "- Both IDs must have source-ref-backed events, but the two IDs must not share a source_ref or event; otherwise the row belongs to an earlier or separate gate.",
        "- Both IDs must share at least two venue anchors.",
        ...
        json.dumps(
            {
                ...
                "initial_eligible_candidates": report["initial_state"]["unicode_multivenue_candidate_count"],
                "final_eligible_candidates": report["final_state"]["unicode_multivenue_candidate_count"],
