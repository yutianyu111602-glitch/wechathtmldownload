#!/usr/bin/env python3
"""Build a report-only source-ref collection queue for high-risk DB3 DJ identities."""
...
def stable_source_ref_task_id(row: dict[str, Any]) -> str:
    payload = {
    ...
    return f"djsrcref:{digest}"


def build_source_ref_row(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "source_ref_task_id": stable_source_ref_task_id(row),
        "review_item_id": row.get("review_item_id", ""),
...
def build_source_ref_queue(review_queue: dict[str, Any], *, source_review_queue: Path) -> dict[str, Any]:
    rows = [row for row in as_list(review_queue.get("review_queue_rows")) if isinstance(row, dict)]
    source_ref_rows = [build_source_ref_row(row) for row in rows if row.get("next_action") == SOURCE_REF_ACTION]
    source_ref_rows.sort(key=lambda row: (row["review_order"], -row["priority_score"], row["source_ref_task_id"]))
    lineup_rows = [row for row in rows if row.get("next_action") == LINEUP_ACTION]
    reason_counts = Counter(reason for row in source_ref_rows for reason in row["reason_codes"])
    batch_counts = Counter(str(row.get("review_batch_id") or "") for row in source_ref_rows)
    task_ids = [row["source_ref_task_id"] for row in source_ref_rows]
    return {
        "schema_version": "atlas_dj_identity_source_ref_collection_queue.v1",
        "generated_at": datetime.now(timezone.utc).astimezone().isoformat(timespec="seconds"),
        "decision": "atlas_dj_identity_source_ref_collection_queue_ready_report_only",
        ...
        "lineup_confirmation_row_count": len(lineup_rows),
        "source_ref_task_ids_unique": len(task_ids) == len(set(task_ids)),
        ...
        "database_write_allowed": False,
        "source_ref_queue_rows": source_ref_rows,
        "rules": [
            "This queue collects source refs or original URLs only; it does not fetch, scrape, or call providers.",
            "Rows remain unapproved until source refs and field preservation notes are filled and revalidated.",
            "Blank collection fields must not overwrite existing DB1/DB2/DB3 non-empty fields.",
        ],
        "safety": {
            "report_only": True,
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# Atlas DJ Identity Source Ref Collection Queue",
        "",
        f"- Decision: `{report['decision']}`",
        f"- Source review queue: `{report['source_review_queue']}`",
        f"- Source-ref rows: `{report['source_ref_queue_row_count']}`",
        f"- Lineup confirmation rows excluded: `{report['lineup_confirmation_row_count']}`",
        f"- Source-ref task ids unique: `{str(report['source_ref_task_ids_unique']).lower()}`",
        "",
        "## Source Ref Queue Summary",
        "",
        "### Reason Counts",
    ]
...
def write_outputs(report: dict[str, Any], out_dir: Path) -> dict[str, Path]:
    ...
    json_path = out_dir / "atlas_dj_identity_source_ref_collection_queue.json"
    rows_path = out_dir / "atlas_dj_identity_source_ref_collection_queue_rows.jsonl"
    md_path = out_dir / "atlas_dj_identity_source_ref_collection_queue.md"
    write_json(json_path, report)
    rows_path.write_text(
        "\n".join(json.dumps(row, ensure_ascii=False, sort_keys=True) for row in report["source_ref_queue_rows"]) + "\n",
        encoding="utf-8",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_source_ref_queue(review_queue, source_review_queue=args.review_queue)
    paths = write_outputs(report, args.out_dir)
    summary = {
        "decision": report["decision"],
        "source_ref_queue_row_count": report["source_ref_queue_row_count"],
        "lineup_confirmation_row_count": report["lineup_confirmation_row_count"],
        "source_ref_task_ids_unique": report["source_ref_task_ids_unique"],
        "json": str(paths["json"]),
        "rows": str(paths["rows"]),
