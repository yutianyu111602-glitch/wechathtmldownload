...
# 门洞/我只会为你心动: curated venue whose film screenings count as events (user opt-in).
# Film terms only grant event_signal for this fakeid; other accounts' films stay filtered.
CURATED_FILM_FAKEIDS = {"MzU0ODY3NjcyMw=="}
FILM_EVENT_RE = re.compile(r"放映|纪录片|展映|影展|观影", re.I)
...
def build_candidate(row: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    if first_string(row.get("account_fakeid")) in CURATED_FILM_FAKEIDS and FILM_EVENT_RE.search(text):
        event_signal = True
