#!/usr/bin/env python3
"""Run S195 DB3 identity batch for source-title/date/city cluster chains.

S195 consumes the fresh S195 cluster-chain queue. It is intentionally broader
than the older two-profile gates, but still narrow: every profile in the live
cluster must share one source account, one source/event title, one event date,
and one city anchor. Venue ids/names may differ because the historical DB has
split venue records for the same party provider.
"""
...
CURRENT_STORY_ID = "S195"
REPORT_STEM = "atlas_relation_identity_cluster_chain_batch_s195"
SCHEMA_VERSION = f"{REPORT_STEM}.v1"

DEFAULT_DB3 = REPO_ROOT / "services" / "weekly_activity_cloudrun" / "data" / "atlas_miniapp.sqlite"
DEFAULT_INPUT = (
    REPORTS_ROOT
    / "atlas_relation_identity_source_provider_acquisition_s195_after_s194_full_20260602"
    / "cluster_chain_rows_s185.jsonl"
)
DEFAULT_OUT_DIR = REPORTS_ROOT / "atlas_relation_identity_cluster_chain_batch_s195_20260602"
DEFAULT_SCORECARD = REPO_ROOT / "reports" / "WEEKLY_ATLAS_RELATION_IDENTITY_CLUSTER_CHAIN_BATCH_S195_20260602.md"

ALLOWED_REASONS = {"group_size_not_two", "city_empty", "token_not_safe_ascii", "display_token_mismatch"}
MIN_CLUSTER_SIZE = 3
MAX_CLUSTER_SIZE = 8
CITY_POLICY = "preserve_non_empty_city_or_require_common_city_chain_anchor"
...
def group_lookup(conn: sqlite3.Connection) -> dict[frozenset[str], tuple[str, str, list[dict[str, Any]]]]:
    return s189.group_lookup(conn)


def _values_for_compact(selected: dict[str, Any]) -> list[str]:
    return [
...
def build_live_candidate(
    ...
    blockers: list[str] = []
    if row.get("s185_route") != "cluster_chain_evidence_ready":
        blockers.append("not_cluster_chain_evidence_ready_route")
    ...
    has_common_city = bool(intersections["common_cities"])
    ...
    if not (MIN_CLUSTER_SIZE <= len(group) <= MAX_CLUSTER_SIZE):
        blockers.append("cluster_size_outside_s195_bounds")
    ...
    if not has_common_city:
        blockers.append("same_city_chain_anchor_missing")
    ...
    candidate["prewrite_row_id"] = f"s195:{s159.group_hash(city, token, sorted(dj_ids))}"
    candidate["s185_group_id"] = row.get("group_id")
    candidate["s185_route"] = row.get("s185_route")
    candidate["s195_common_evidence"] = intersections
    candidate["s195_city_values"] = sorted(city_values)
    candidate["s159_rejection_reasons_before_s195"] = classified.get("s159_rejection_reasons") or []
    ...
    candidate["s195_risk_reasons"] = risk_reasons
    candidate["safety_basis"] = [
        "s185_cluster_chain_evidence_ready_route",
        ...
        "same_city_chain_anchor",
        "city_context_has_no_conflict",
        "venue_split_allowed_only_when_source_title_date_city_chain_is_common",
...
def live_candidates(
    ...
    state = {
        "input_cluster_chain_count": len(rows),
        "cluster_chain_candidate_count": len(candidates),
        "rejected_count": len(rejected),
        "rejection_reason_counts": dict(Counter(reason for row in rejected for reason in row.get("s195_blockers") or [])),
        "table_counts": s148.table_counts(conn),
    }
    return sorted(candidates, key=lambda item: tuple(item["risk_tuple_live"])), rejected, state


def validate_s195_prewrite(conn: sqlite3.Connection, selected: dict[str, Any]) -> list[str]:
    blockers: list[str] = []
    if selected.get("city_policy") != CITY_POLICY:
        blockers.append("city_policy_missing")
    if not set(selected.get("s159_rejection_reasons_before_s195") or []) <= ALLOWED_REASONS:
    ...
    evidence = selected.get("s195_common_evidence") or {}
    ...
    if not evidence.get("common_starts_at") and not evidence.get("common_post_dates"):
        blockers.append("same_starts_at_or_post_date_missing")
    if not evidence.get("common_cities"):
        blockers.append("same_city_chain_anchor_missing")
    if not s189.city_anchor_ok(set(selected.get("s195_city_values") or []), evidence.get("common_cities") or []):
        blockers.append("city_context_conflict")
    if len(s167.compact_set(_values_for_compact(selected))) != 1:
        blockers.append("unicode_compact_not_equivalent")
    if s171.s171_risk_reasons(_values_for_compact(selected)) or s167.has_enhanced_risk(_values_for_compact(selected)):
        blockers.append("enhanced_risk_or_collaboration_marker")
    ...
    *,
    ...
    with s148.FileLock(lock_path, max_attempts=max_lock_attempts):
        if shared_backup:
            ...
            backup_path = out_dir / f"backup_before_relation_identity_cluster_chain_{story_label}_{stamp}.sqlite"
        ...
        try:
            pre_blockers = validate_s195_prewrite(conn, selected)
            if pre_blockers:
...
def run_batch(
    *,
    ...
    if execute and batch_backup:
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_path = out_dir / f"backup_before_relation_identity_cluster_chain_batch_s195_{stamp}.sqlite"
        shutil.copy2(db3_path, backup_path)
    ...
    for index in range(max_count):
        ...
        if not queue:
            ...
            break
        selected = queue[0]
        story_label = f"s195_{index + 1:03d}"
        iteration: dict[str, Any] = {
            ...
            "s195_common_evidence": selected["s195_common_evidence"],
    ...
    report = {
        ...
        "artifacts": {
            ...
            "initial_candidates_jsonl": rel_path(out_dir / "cluster_chain_initial_candidates_s195.jsonl"),
            "initial_rejected_jsonl": rel_path(out_dir / "cluster_chain_initial_rejected_s195.jsonl"),
            "final_candidates_jsonl": rel_path(out_dir / "cluster_chain_final_candidates_s195.jsonl"),
            "final_rejected_jsonl": rel_path(out_dir / "cluster_chain_final_rejected_s195.jsonl"),
        },
    }
    write_jsonl(out_dir / "cluster_chain_initial_candidates_s195.jsonl", initial_candidates)
    write_jsonl(out_dir / "cluster_chain_initial_rejected_s195.jsonl", initial_rejected)
    write_jsonl(out_dir / "cluster_chain_final_candidates_s195.jsonl", final_candidates)
    write_jsonl(out_dir / "cluster_chain_final_rejected_s195.jsonl", final_rejected)
...
def write_scorecard(path: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        "# Weekly Atlas Relation Identity Cluster Chain Batch S195",
        "",
        f"- Decision: `{report['decision']}`",
        f"- Initial cluster-chain candidates: `{initial['cluster_chain_candidate_count']}`",
        f"- Committed: `{report['committed_count']}`",
        f"- Final cluster-chain candidates: `{final['cluster_chain_candidate_count']}`",
        ...
        "- Cluster-chain acceptance requires common source account, common source/event title, common starts_at/post_date, and common city anchor across the full live cluster.",
        "- Venue/provider id splits are tolerated only when the title/date/city/source-account chain is common across all profiles.",
        ...
        json.dumps(
            {
                ...
                "initial_candidates": report["initial_state"]["cluster_chain_candidate_count"],
                "final_candidates": report["final_state"]["cluster_chain_candidate_count"],
