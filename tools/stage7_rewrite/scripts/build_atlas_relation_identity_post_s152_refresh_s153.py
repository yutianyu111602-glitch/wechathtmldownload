#!/usr/bin/env python3
"""Refresh the DB3 relation-identity queue after the S152 canary."""
...
CURRENT_STORY_ID = "S153"
SCHEMA_VERSION = "atlas_relation_identity_post_s152_refresh_s153.v1"

DEFAULT_S148_REPORT = REPORTS_ROOT / "atlas_relation_identity_db3_canary_s148_20260602" / "atlas_relation_identity_db3_canary_s148.json"
DEFAULT_S150_REPORT = REPORTS_ROOT / "atlas_relation_identity_db3_canary_s150_20260602" / "atlas_relation_identity_db3_canary_s150.json"
DEFAULT_S152_REPORT = REPORTS_ROOT / "atlas_relation_identity_db3_canary_s152_20260602" / "atlas_relation_identity_db3_canary_s152.json"
...
DEFAULT_OUT_DIR = REPORTS_ROOT / "atlas_relation_identity_post_s152_refresh_s153_20260602"
DEFAULT_SCORECARD = REPO_ROOT / "reports" / "WEEKLY_ATLAS_RELATION_IDENTITY_POST_S152_REFRESH_S153_20260602.md"
...
def carry_forward_gap_row(row: dict[str, Any]) -> dict[str, Any]:
    carried = s151.carry_forward_gap_row(row)
    carried["s153_excluded_row_id"] = carried.pop("s151_excluded_row_id", f"s153-gap:{s149.stable_id(row)}")
    carried["source"] = "s145_gap_row_carried_forward_via_s153"
    return carried
...
def build_tasks(report: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        tasks.append(
            {
                ...
                "task_id": "s153:prior_canary_closure_failed",
                "status": "blocked",
                "next_action": "Audit S148/S150/S152 live DB3 readback before selecting another identity write.",
            }
        )
    if next_canary:
        tasks.append(
            {
                "story_id": CURRENT_STORY_ID,
                "task_id": "s153:next_canary_gate_ready_report_only",
                "status": "next",
                "group_id": next_canary.get("group_id"),
                "source_prewrite_row_id": next_canary.get("source_prewrite_row_id"),
                "next_action": "Build S154 single-row approval/execution gate from this S153 row; keep lock, backup, rollback, no-empty-overwrite, and postwrite readback bound to live DB3.",
            }
        )
    else:
        tasks.append(
    ...
    tasks.append(
        {
            "story_id": CURRENT_STORY_ID,
            "task_id": "s153:preserve_non_relation_blockers",
...
def refresh_row_for_s153(
    ...
    *,
    ...
    refreshed = s151.refresh_row_for_s151(
        conn,
        row,
    ...
    refreshed["s153_queue_row_id"] = f"s153:{s149.stable_id([prewrite_id, refreshed.get('live_affected_row_estimate')])}"
    return refreshed
...
def build_report(
    *,
    ...
    reports = {
        "S148": (
            s149.read_json(s148_report_path),
            s148_report_path,
            "atlas_relation_identity_db3_canary_s148_committed_with_readback",
        ),
        "S150": (
            s149.read_json(s150_report_path),
            s150_report_path,
            "atlas_relation_identity_db3_canary_s150_committed_with_readback",
        ),
        "S152": (
            s149.read_json(s152_report_path),
            s152_report_path,
            "atlas_relation_identity_db3_canary_s152_committed_with_readback",
        ),
    }
    s146_rows = s149.read_jsonl(s146_rows_path)
    gap_rows = s149.read_jsonl(s146_gaps_path)
    executed_by_prewrite_id = {
        str(s151.selected_from_report(report).get("prewrite_row_id")): story
        for story, (report, _path, _decision) in reports.items()
        if s151.selected_from_report(report).get("prewrite_row_id")
    }
    ...
    try:
        ...
        closures = {
            story: s151.prove_canary_closure(
                conn,
                report=report,
            ...
            for story, (report, _path, expected_decision) in reports.items()
        }
        refreshed_rows = [
            refresh_row_for_s153(
                conn,
                row,
                executed_prewrite_ids=executed_prewrite_ids,
                executed_by_prewrite_id=executed_by_prewrite_id,
    ...
    decision = (
        "atlas_relation_identity_post_s152_refresh_s153_ready_next_canary_report_only"
        if all_closed and next_canary
        else "atlas_relation_identity_post_s152_refresh_s153_blocked_report_only"
    )
    report: dict[str, Any] = {
        ...
        "source_inputs": {
            ...
            "s152_report": rel_path(s152_report_path),
            "s152_report_sha256": s149.sha256_file(s152_report_path),
        ...
        "rules": [
            "S153 recomputes affected rows and risks from live DB3 after S152.",
            "S148/S150/S152 selected rows and S145 gap rows are excluded from further write execution.",
...
def render_markdown(report: dict[str, Any], paths: dict[str, str]) -> str:
    ...
    lines = [
        "# Atlas Relation Identity Post-S152 Refresh S153",
        "",
    ...
    lines.extend(
        [
            ...
            "## Boundary",
            "",
            "- S153 is read-only and report-local.",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--s152-report", type=Path, default=DEFAULT_S152_REPORT)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        s152_report_path=args.s152_report,
    ...
    paths = {
        "json": rel_path(args.out_dir / "atlas_relation_identity_post_s152_refresh_s153.json"),
        "queue": rel_path(args.out_dir / "relation_identity_next_canary_queue_s153.jsonl"),
        "excluded": rel_path(args.out_dir / "relation_identity_excluded_rows_s153.jsonl"),
        "tasks": rel_path(args.out_dir / "relation_identity_post_s152_tasks_s153.jsonl"),
        "markdown": rel_path(args.out_dir / "atlas_relation_identity_post_s152_refresh_s153.md"),
        "scorecard": rel_path(args.scorecard),
    }
    atomic_write_json(args.out_dir / "atlas_relation_identity_post_s152_refresh_s153.json", report)
    atomic_write_jsonl(args.out_dir / "relation_identity_next_canary_queue_s153.jsonl", report["next_canary_queue_sample"])
    atomic_write_jsonl(
        args.out_dir / "relation_identity_excluded_rows_s153.jsonl",
        report["excluded_candidate_sample"] + report["s145_gap_rows_carried_forward"],
    )
    atomic_write_jsonl(args.out_dir / "relation_identity_post_s152_tasks_s153.jsonl", report["next_action_tasks"])
    atomic_write_text(args.out_dir / "atlas_relation_identity_post_s152_refresh_s153.md", render_markdown(report, paths))
