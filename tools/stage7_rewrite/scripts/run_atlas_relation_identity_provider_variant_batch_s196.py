#!/usr/bin/env python3
"""Run S196 DB3 identity batch for strict provider/venue name variants.

S196 targets the S196 provider-crosscheck queue after S195. It accepts only
multi-profile groups that share a provider/source account and venue anchor, have
one Unicode compact identity token, no city conflict, and no role/compound
markers. It deliberately does not require a shared event title/date because this
lane handles long-horizon venue/provider identity variants.
"""
...
CURRENT_STORY_ID = "S196"
REPORT_STEM = "atlas_relation_identity_provider_variant_batch_s196"
SCHEMA_VERSION = f"{REPORT_STEM}.v1"

DEFAULT_DB3 = REPO_ROOT / "services" / "weekly_activity_cloudrun" / "data" / "atlas_miniapp.sqlite"
DEFAULT_INPUT = (
    REPORTS_ROOT
    / "atlas_relation_identity_source_provider_acquisition_s196_after_s195_full_20260602"
    / "provider_crosscheck_rows_s185.jsonl"
)
DEFAULT_OUT_DIR = REPORTS_ROOT / "atlas_relation_identity_provider_variant_batch_s196_20260602"
DEFAULT_SCORECARD = REPO_ROOT / "reports" / "WEEKLY_ATLAS_RELATION_IDENTITY_PROVIDER_VARIANT_BATCH_S196_20260602.md"

ALLOWED_REASONS = {"group_size_not_two", "city_empty", "token_not_safe_ascii", "display_token_mismatch"}
MIN_CLUSTER_SIZE = 3
MAX_CLUSTER_SIZE = 8
CITY_POLICY = "preserve_non_empty_city_or_allow_empty_when_provider_variant_has_no_city_conflict"
ROLE_OR_COMPOUND_RE = re.compile(
    r"(\baka\b|/|\\|&|\+|\bb2b\b|\bvs\b|\bfeat\b|\bft\b| x |\bvj\b|\[vj\]|\(vj\)|（vj）)",
    re.IGNORECASE,
)
...
def provider_variant_token_blockers(compact_token: str, values: list[str]) -> list[str]:
    blockers: list[str] = []
    if len(compact_token) < 5 and not has_cjk(compact_token):
        blockers.append("compact_token_too_short_for_provider_variant_gate")
    if any(char.isdigit() for char in compact_token):
        blockers.append("digit_token_not_allowed_in_provider_variant_gate")
    if ROLE_OR_COMPOUND_RE.search(" ".join(values)):
...
def build_live_candidate(
    ...
    intersections, contexts = s189.source_evidence_intersections(conn, dj_ids)
    city_values = s189.context_city_values(group, contexts)

    if not (MIN_CLUSTER_SIZE <= len(group) <= MAX_CLUSTER_SIZE):
        blockers.append("cluster_size_outside_s196_bounds")
    ...
    blockers.extend(provider_variant_token_blockers(compact_token, values))
    if risk_reasons or s167.has_enhanced_risk(values):
        blockers.append("enhanced_risk_or_collaboration_marker")
    if not intersections["common_source_accounts"]:
        blockers.append("same_source_account_missing")
    if not (intersections["common_venue_ids"] or intersections["common_venue_names"]):
        blockers.append("same_venue_or_provider_anchor_missing")
    if not all(context.get("event_sample_count", 0) > 0 for context in contexts):
        blockers.append("source_context_missing")
    if not s189.city_anchor_ok(city_values, intersections["common_cities"]):
        blockers.append("city_context_conflict")

    candidate = s159.build_candidate(city, token, group)
    candidate["prewrite_row_id"] = f"s196:{s159.group_hash(city, token, sorted(dj_ids))}"
    candidate["s185_group_id"] = row.get("group_id")
    candidate["s185_route"] = row.get("s185_route")
    candidate["s196_common_evidence"] = intersections
    candidate["s196_city_values"] = sorted(city_values)
    candidate["s159_rejection_reasons_before_s196"] = classified.get("s159_rejection_reasons") or []
    ...
    candidate["s196_risk_reasons"] = sorted(set([*risk_reasons, *provider_variant_token_blockers(compact_token, values)]))
...
def live_candidates(
    ...
    state = {
        ...
        "provider_variant_candidate_count": len(candidates),
        "rejected_count": len(rejected),
        "rejection_reason_counts": dict(Counter(reason for row in rejected for reason in row.get("s196_blockers") or [])),
        "table_counts": s148.table_counts(conn),
    }
    return sorted(candidates, key=lambda item: tuple(item["risk_tuple_live"])), rejected, state


def validate_s196_prewrite(conn: sqlite3.Connection, selected: dict[str, Any]) -> list[str]:
    blockers: list[str] = []
    if selected.get("city_policy") != CITY_POLICY:
        blockers.append("city_policy_missing")
    if not set(selected.get("s159_rejection_reasons_before_s196") or []) <= ALLOWED_REASONS:
    ...
    if not s169.profile_evidence_ok(selected):
        blockers.append("profile_source_ref_or_subject_evidence_missing")
    evidence = selected.get("s196_common_evidence") or {}
    if not evidence.get("common_source_accounts"):
        blockers.append("same_source_account_missing")
    if not evidence.get("common_venue_ids") and not evidence.get("common_venue_names"):
        blockers.append("same_venue_or_provider_anchor_missing")
    if not s189.city_anchor_ok(set(selected.get("s196_city_values") or []), evidence.get("common_cities") or []):
        blockers.append("city_context_conflict")
    ...
    *,
    ...
    with s148.FileLock(lock_path, max_attempts=max_lock_attempts):
        if shared_backup:
            ...
            backup_path = out_dir / f"backup_before_relation_identity_provider_variant_{story_label}_{stamp}.sqlite"
        ...
        try:
            pre_blockers = validate_s196_prewrite(conn, selected)
            if pre_blockers:
...
def run_batch(
    *,
    ...
    if execute and batch_backup:
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_path = out_dir / f"backup_before_relation_identity_provider_variant_batch_s196_{stamp}.sqlite"
        shutil.copy2(db3_path, backup_path)
    ...
    for index in range(max_count):
        ...
        if not queue:
            ...
            break
        selected = queue[0]
        story_label = f"s196_{index + 1:03d}"
        iteration: dict[str, Any] = {
            ...
            "s196_common_evidence": selected["s196_common_evidence"],
    ...
    report = {
        ...
        "artifacts": {
            ...
            "initial_candidates_jsonl": rel_path(out_dir / "provider_variant_initial_candidates_s196.jsonl"),
            "initial_rejected_jsonl": rel_path(out_dir / "provider_variant_initial_rejected_s196.jsonl"),
            "final_candidates_jsonl": rel_path(out_dir / "provider_variant_final_candidates_s196.jsonl"),
            "final_rejected_jsonl": rel_path(out_dir / "provider_variant_final_rejected_s196.jsonl"),
        },
    }
    write_jsonl(out_dir / "provider_variant_initial_candidates_s196.jsonl", initial_candidates)
    write_jsonl(out_dir / "provider_variant_initial_rejected_s196.jsonl", initial_rejected)
    write_jsonl(out_dir / "provider_variant_final_candidates_s196.jsonl", final_candidates)
    write_jsonl(out_dir / "provider_variant_final_rejected_s196.jsonl", final_rejected)
...
def write_scorecard(path: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        "# Weekly Atlas Relation Identity Provider Variant Batch S196",
        "",
        f"- Decision: `{report['decision']}`",
        f"- Initial provider-variant candidates: `{initial['provider_variant_candidate_count']}`",
        f"- Committed: `{report['committed_count']}`",
        f"- Final provider-variant candidates: `{final['provider_variant_candidate_count']}`",
        ...
        "- Provider-variant acceptance requires same source account plus same venue/provider anchor, one compact token, and no city conflict.",
        "- It excludes digit/short tokens and aka/slash/b2b/feat/vj/compound markers.",
        ...
        json.dumps(
            {
                ...
                "initial_candidates": report["initial_state"]["provider_variant_candidate_count"],
                "final_candidates": report["final_state"]["provider_variant_candidate_count"],
